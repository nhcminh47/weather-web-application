...
import type { GeoError } from '../types/weather'
...
const useDeviceLocation = (): UseDeviceLocationResult => {
  const [location, setLocation] = useState<{ value: { lat: number; lon: number } } | null>(null)
  ...
  const success = (pos: GeolocationPosition) => {
    setLocation({ value: { lat: pos.coords.latitude, lon: pos.coords.longitude } })
